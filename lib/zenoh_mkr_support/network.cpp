...
z_result_t _z_open_serial_from_pins(_z_sys_net_socket_t *sock, uint32_t txpin, uint32_t rxpin, uint32_t baudrate) {
    ...
    if (rxpin == PIN_SERIAL1_RX && txpin == PIN_SERIAL1_TX) {
        uart = 1;
    } else if (rxpin == PIN_SERIAL2_RX && txpin == PIN_SERIAL2_TX) {
        uart = 2;
    } else {
        ret = _Z_ERR_GENERIC;
    }
...
z_result_t _z_open_serial_from_dev(_z_sys_net_socket_t *sock, char *dev, uint32_t baudrate) {
    ...
    auto res = _z_connect_serial(*sock);
    return res;
}
...
size_t _z_read_serial_internal(const _z_sys_net_socket_t sock, uint8_t *header, uint8_t *ptr, size_t len) {
    ...
    Serial.println("Rxd serial data...");
    if (ret == SIZE_MAX) {
        Serial.println("Error deserializing serial data");
        return ret;
    }
    Serial.println("Deserialized serial data successfully");
    Serial.print("Header: ");
    ...
    Serial.println(ret);
    Serial.println("End of serial data");
    Serial.println();
